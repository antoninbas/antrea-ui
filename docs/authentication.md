...
| --- | --- | --- |
| No request for a while | 30 minutes | `auth.session.idleTimeout` |
| Absolute age | 12 hours | `auth.session.maxLifetime` |
| The credential expires and cannot be renewed | — | — |
| Logout, in this tab or another | — | — |
| The same user logs in from too many places | 10 concurrent | `auth.session.maxSessionsPerUser` |
...
The store holds at most `auth.session.maxSessions` sessions (default 1000), and
at most `auth.session.maxSessionsPerUser` of those for any one identity (default
