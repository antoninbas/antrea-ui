...
## Built-in Dex has been removed

Earlier releases could run Dex as a sidecar in the `antrea-ui` Pod, configured
through `dex.*` Helm values. That is gone, and setting any `dex.*` value now
makes `helm install` fail with an explanatory message.

The reason is the precondition above. Built-in Dex listened on `localhost:5556`
inside the Pod, which the kube-apiserver has no route to and no reason to trust
as an issuer. That was fine when the id_token was only used to authenticate to
Antrea UI itself, but it is now the credential Antrea UI presents to Kubernetes,
so an issuer the apiserver does not trust cannot work at all.

If you were using built-in Dex, you have two options:

* **Run Dex yourself**, as a normal Deployment at an address the kube-apiserver
  can reach, and point both the apiserver and Antrea UI at it. Your existing
  `dex.config.connectors` carry over to that Dex instance unchanged. Then follow
  [Configuring an OIDC provider](#configuring-an-oidc-provider) below.
  `ci/e2e-oidc.sh` is a complete worked example of this arrangement — it is what
  the OIDC e2e test runs against — including the apiserver flags, the CA
  plumbing, and how to make one issuer URL resolve from the apiserver, from Pods,
  and from a browser.
* **Use a different login mode.** If configuring the apiserver for OIDC is not
  practical, the Kubernetes token
  (`auth.serviceAccountToken.enable`) and kubeconfig (`auth.kubeconfig.enable`)
  modes also give each user their own Kubernetes identity, with no identity
  provider involved. See [authentication.md](authentication.md).
