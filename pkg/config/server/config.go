...
func validateConfig(config *Config) error {
	...
	if config.Auth.Session.IdleTimeout <= 0 {
		return fmt.Errorf("auth.session.idleTimeout must be positive")
	}
	if config.Auth.Session.MaxLifetime <= 0 {
		return fmt.Errorf("auth.session.maxLifetime must be positive")
	}
	if config.Auth.Session.MaxLifetime < config.Auth.Session.IdleTimeout {
		return fmt.Errorf("auth.session.maxLifetime must be >= auth.session.idleTimeout")
	}
	if config.Auth.Session.MaxSessions <= 0 {
		return fmt.Errorf("auth.session.maxSessions must be positive")
	}
	if config.Auth.Session.MaxSessionsPerUser <= 0 {
		return fmt.Errorf("auth.session.maxSessionsPerUser must be positive")
	}
	if config.Auth.Session.MaxSessionsPerUser > config.Auth.Session.MaxSessions {
		return fmt.Errorf("auth.session.maxSessionsPerUser must be <= auth.session.maxSessions")
	}
...
func LoadConfig() (*Config, error) {
	...
	v.SetDefault("auth.session.idleTimeout", DefaultSessionIdleTimeout)
	v.SetDefault("auth.session.maxLifetime", DefaultSessionMaxLifetime)
	v.SetDefault("auth.session.maxSessions", DefaultMaxSessions)
	v.SetDefault("auth.session.maxSessionsPerUser", DefaultMaxSessionsPerUser)
